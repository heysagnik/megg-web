...
export default function ProductsClient({
  ...
  const initialFiltersState = useMemo<BrowseFilters>(
    () => ({ ...BROWSE_EMPTY, subcategory: urlCategory }),
    [urlCategory],
  )

  const [products, setProducts]   = useState<Product[]>(initialProducts)
  const [total, setTotal]         = useState<number>(initialTotal)
  const [hasMore, setHasMore]     = useState<boolean>(initialProducts.length === PAGE_SIZE && !urlCategory)
  const [page, setPage]           = useState<number>(1)
  const [filters, setFilters]     = useState<BrowseFilters>(initialFiltersState)
  ...
  useLayoutEffect(() => {
    ...
    const cached = readBrowseCache(CACHE_KEY, pathname, search)
  ...
  const fetchPage = useCallback(async (pageNum: number, f: BrowseFilters, reset: boolean) => {
    ...
    try {
      ...
      const shouldApplyAvail = (reset && noFilters) || (reset && f.subcategory)
      if (shouldApplyAvail && res.availableFilters) setAvail(res.availableFilters)
      setProducts(prev => {
  ...
  // ── sessionStorage persistence (debounced on scroll + on unmount) ──────
  useEffect(() => {
    ...
    const onScroll = () => {
      clearTimeout(t)
      t = setTimeout(() => {
        writeBrowseCache(CACHE_KEY, pathname, search, {
  ...
  }, [pathname, search, products, total, hasMore, page, filters, avail])
  ...
  // ── Auto-hide mobile bars on scroll ────────────────────────────────────
  useEffect(() => {
  ...
  // ── Hide sticky mobile bar when footer is visible ──────────────────────
  useEffect(() => {
